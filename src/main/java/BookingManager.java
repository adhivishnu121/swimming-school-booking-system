...
public class BookingManager {
	...
	private void addSampleBookings() {
        ...
        for (int i = 0; i < 4; i++) { // Repeat for 4 weeks
            ...
            LocalDate nextSaturday = currentDate.with(TemporalAdjusters.nextOrSame(DayOfWeek.SATURDAY)).plusWeeks(i);

            bookings.add(new Booking(learners.get(0), nextMonday.toString(), "16:00", coaches.get(0), 3, generateLessonId()));
            bookings.add(new Booking(learners.get(1), nextMonday.toString(), "17:00", coaches.get(0), 0, generateLessonId()));
            bookings.add(new Booking(learners.get(2), nextMonday.toString(), "18:00", coaches.get(0), 2, generateLessonId()));
	...
	public void displayTimetable(List<Booking> bookings) {
		...
		for (Booking booking : bookings) {
			System.out.printf("| %-10s | %-10s | %-10s | %-15s | %-10s |\n", booking.getLessonDate(),
					booking.getLessonTime(), booking.getCoach().getName(),
					(booking.getLearner() != null ? booking.getLearner().getName() : ""), booking.getGradeLevel());
		}
		System.out.println("-------------------------------------------------------------------------");
	}
