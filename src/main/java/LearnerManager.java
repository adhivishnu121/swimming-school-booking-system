...
class LearnerManager {
    ...
    public void registerLearner(List<Learner> learners) {
        ...
        // Save learner record to the system
...
public void monthlyReport(List<Booking> bookings) {
    ...
    for (Booking booking : bookings) {
        ...
        if (bookingMonth == month) {
            ...
            String bookingDetails = String.format("%s lesson on %s at %s with coach %s", 
                                    status, booking.getLessonDate(), 
                                    booking.getLessonTime(), booking.getCoach().getName());

            // Update learner bookings
    ...
    // Generate and display the monthly learner report
    ...
    for (String learnerName : learnerBookings.keySet()) {
        ...
        System.out.printf("%-14s | %-6d | %-9d | %-8d | %s\n", learnerName, totalBooked, totalCancelled, totalAttended, bookingsList);
