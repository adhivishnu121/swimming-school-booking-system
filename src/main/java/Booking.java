...
public class Booking {
    ...
    public Booking(Learner learner, String lessonDate, String lessonTime, Coach coach, int gradeLevel, int lessonId) {
        ...
        this.status = "booked"; // Initialize status as "booked" by default
        this.review = ""; // Initialize review as an empty string
        this.rating = 0; // Initialize rating as 0
        this.coach = coach;
        this.gradeLevel = gradeLevel;
      // this.MAX_CAPACITY = 4; // Maximum capacity for each lesson
        this.currentCapacity = 1; // Initial capacity with one learner
    }
    ...
    public String getTime() {
        return lessonTime; // Return the lesson time
    }
       
    public DayOfWeek getDayOfWeek() {
    	LocalDate date = LocalDate.parse(lessonDate);
        
        // Check if the lesson date falls on Monday, Wednesday, Friday, or Saturday
        if (date.getDayOfWeek() == DayOfWeek.MONDAY ||
            date.getDayOfWeek() == DayOfWeek.WEDNESDAY ||
            date.getDayOfWeek() == DayOfWeek.FRIDAY) {
            return date.getDayOfWeek();
        } else if (date.getDayOfWeek() == DayOfWeek.SATURDAY) {
            // Additional check for Saturday to ensure the lesson time is within the specified slots
        ...
        // Return null or throw an exception for invalid lesson dates
        return null; // Placeholder
    }
   

	public void addLearner(Learner learner) {
        // Check if there is still capacity for the learner to be added
        if (currentCapacity < MAX_CAPACITY) {
            // Increase the current capacity
            increaseCapacity();
            // Print a message indicating successful addition
            System.out.println("Learner " + learner.getName() + " added to the booking.");
        } else {
            // Print a message indicating that the booking is full
